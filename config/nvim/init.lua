...
do
  ...
  -- Exit insert mode by pressing i twice
  ...
  -- Detect Terraform when an unnamed buffer is first saved with a Terraform extension.
  vim.api.nvim_create_autocmd({ 'BufFilePost', 'BufWritePost' }, {
    ...
    callback = function(event)
      local filetype = event.file:match '%.tfvars$' and 'terraform-vars' or 'terraform'
      if vim.bo[event.buf].filetype ~= filetype then vim.bo[event.buf].filetype = filetype end
    end,
  })
end
...
do
  ...
  local servers = {
    ...
    gopls = {
      settings = {
        gopls = {
          gofumpt = true, -- Use stricter gofumpt formatting
          analyses = {
            unusedparams = true,
            shadow = true,
          },
  ...
  -- nvim-lspconfig no longer provides :LspInfo on Neovim 0.12.
...
do
  ...
  local parsers = { 'bash', 'c', 'diff', 'html', 'lua', 'luadoc', 'markdown', 'markdown_inline', 'query', 'vim', 'vimdoc', 'terraform', 'go', 'gomod', 'gosum' }
